...
A user can revoke an `access_token` or `refresh_token` using the following endpoint. Once revoked, the token will be seen as inactive by the introspection endpoint.
...
The manage endpoint directs end-user's to a URL where they can view applications which have access to their data. Users can adjust these application's access rights from this URL as well.
