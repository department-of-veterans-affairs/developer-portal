...
We added new elements and values to the R4 `Condition` FHIR mapping. 
  ...
  -  The `verificationStatus` element now returns these new values:`provisional`, `differential`, `confirmed`, or `refuted` in addition to the previously returned value, `unconfirmed`.
