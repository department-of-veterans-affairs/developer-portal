...
1. Add data for your API in `src/apiDefs/data`.
    2. If your new API belongs to an existing API category (Benefits, Facilities, Health, or Veteran Verification), then simply add your API definition to the Typescript file for that category in `src/apiDefs/data` (e.g., `health.ts` for the Health API).
    ...
    2. Guidelines for (some) `IApiDescription` properties
