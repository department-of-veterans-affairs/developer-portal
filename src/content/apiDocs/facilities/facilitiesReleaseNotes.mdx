...
- Data about drive times now comes directly from VHA. Responses may vary slightly from the previous implementation because of this switch.
- The response schema has also changed. Instead of returning facility data from `/nearby`, the endpoint now provides `min_time` and `max_time` attributes for the matching polygons and links to endpoints that can be queried to retrieve full information about the associated facility. See the schema changes by [visiting the documentation](https://developer.va.gov/explore/facilities/docs/facilities). 
