### January 10, 2022
  -  Changed the `referenceRange` returned with an `Observation` resource.
      - API no longer returns the field `high` or `low` if there is not an associated value.
      - API will only include the `system`, `code`, and `unit` if all are present.   
