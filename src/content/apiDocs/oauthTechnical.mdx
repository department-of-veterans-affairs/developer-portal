...
The JWT's header has two fields `alg` and `kid`. `alg` indicates the algorithm that was used the sign the JWT, and `kid` identifies the key that was used to sign the JWT. Signing keys and associated metadata are accessible from `https://dev-api.va.gov/oauth2/default/.well-known/oauth-authorization-server`.
