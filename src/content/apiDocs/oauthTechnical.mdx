...
| `prompt`        | Optional     | Supported prompts: `login`, `consent` and `none`.<p>If `login` is specified, the user will be forced to provide credentials regardless of session state. If omitted, an existing active session with the identity provider may not require the user to provide credentials.</p><p>Without the `consent` prompt, the user will only be asked to consent to claims once. If `consent` is specified, the user will be asked to consent to their claims being used regardless of prior consent.</p><p>If `none` is specified, an application will attempt an authorization request without user interaction. When the session is invalid or there are claims the user has not consented to, one of the following errors will be thrown: `login_required` or `consent_required`.</p> |
...
The authorization server will respond with an access token. Since the PKCE flow is typically used by applications that cannot safely store information, using the `offline_access` scope is not recommended. Silent authentication can be used to retrieve a new `access_token` safely. A new authentication request can be made using the `none` prompt like this:
...
If the user's session is inactive or there are claims that have not consented to, the redirect will contain one of the following errors: `login_required` or `consent_required`.

```http
GET /redirect?state={state}&error=login_required&error_description=The+client+specified+not+to+prompt%2C+but+the+user+is+not+logged+in. HTTP/1.1
Host: sandbox-api.va.gov
```
