...
Location: https://dev-api.va.gov/oauth2/authorize?
  ...
  &redirect_uri=<yourRedirectURL>%2Fcb
...
This walk-through assumes you have already installed Node.JS and [NPM](https://www.npmjs.com/get-npm), and that you have obtained an OAuth application client\_id and client\_secret with `<yourRedirectURL>/auth/cb` (⚠️Be sure to use the redirect URL that you registered with) as a white-listed redirect_uri. 
...
This anonymous function asynchronously retrieves the client information then starts the express server. Now you should be able to run `node index.js` and navigate to `<yourRedirectURL>` in your browser (⚠️Be sure to use the redirect URL that you registered with). You should see 'Hello World!' on your screen. 
...
And that's all you'll need to do to add VA OAuth to a node application. After starting the app up again with `node index.js` navigate to `<yourRedirectURL>/auth` in your browser (⚠️Be sure to use the redirect URL that you registered with). Your browser should then take you through the login flow and return you to a page displaying 'Hello World!'. 
...
Now, after restarting `node index.js` you can first navigate to `<yourRedirectURL>/auth` in your browser (⚠️Be sure to use the redirect URL that you registered with) to login, then navigate to `<yourRedirectURL>/status` to see your Veteran Status. 
