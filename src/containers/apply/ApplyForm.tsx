...
const mapStateToProps = (state: RootState): DevApplication => ({
  ...state.application,
});

const renderError = (props: ApplyProps): JSX.Element | null => {
...
const selectedApis = (props: ApplyProps): string[] => {
  const { apis } = props.inputs;
  return Object.keys(apis).filter(apiName => apis[apiName]);
};

const anyOAuthApisSelected = (props: ApplyProps): boolean => {
...
const anyApiSelected = (props: ApplyProps): boolean => {
  const numSelected = selectedApis(props).length;
  return numSelected > 0;
};

const allBioFieldsComplete = (props: ApplyProps): boolean => {
...
const readyToSubmit = (props: ApplyProps): boolean => {
  const {
