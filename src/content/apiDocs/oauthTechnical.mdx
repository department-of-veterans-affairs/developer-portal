...
```
(⚠️Be sure to replace `<yourRedirectURIPath>` with the redirect URI path that you provided during registration, and `<yourRedirectHost>` with your host name)
...
Now, after restarting `node index.js` you can first navigate to `<yourRedirectURL>/auth` in your browser (⚠️Be sure to replace `<yourRedirectURL>` with the redirect URL that you provided during registration) to login, then navigate to `<yourRedirectURL>/status` to see your Veteran Status. 
