...
| `prompt`        | Optional     | Will engage authentication prompts. If this includes `login` it will force a client user to re-enter credentials when prompted at the user authentication site such as ID.me. If this is omitted the user will not be forced to re-enter credentials during authentication. |
