...
-- Tamara Ellis' previous individual code of `A1Q` can still be used, but attempts to assign `A1Q` will not persist.  Using `067` will persist the POA change.
...
-- John Doe's previous individual code of `A1H` can still be used, but attempts to assign `A1H` will not persist.  Using `072` will persist the POA change.
