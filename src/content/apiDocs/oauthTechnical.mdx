...
A user will only be prompted to consent to a particular client to use their data once; however, it is possible to revoke these `grants` for a specified user and client in the Sandbox and Development environments with the following endpoint:
...
Both the `email` address of the user and the `client_id` of the application must be passed into the body of the request. In the case of a bad request, an error response will be returned with a description of the error.
