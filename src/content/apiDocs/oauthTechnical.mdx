...
We support [Authorization Code Flow](#authorization-code-flow) with a *client secret* and [Proof Key for Code Exchange](#proof-key-for-code-exchange-pkce) (PKCE) for public apps.
...
This flow is used by applications that are able to safely store a *client secret* such as a server-based applications.
...
This flow ([RFC 7636](https://tools.ietf.org/html/rfc7636)) is used by public applications that are unable to safely store a *client secret* such as a navite mobile app.
