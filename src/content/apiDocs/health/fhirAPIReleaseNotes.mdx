### November 16, 2021
`Pracitioner` and `PractitionerRole` resources now have new fields.
  -  Practitioner: `qualification.code.text` and `address.period`
  -  PractitionerRole: `code.text`

In Patient Generated Data (PGD) API we have added DELETE interaction for the `QuestionnaireResponse`.
